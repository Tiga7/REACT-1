...
class Student extends Component {
            ...
            $.ajax({
                ...
                success: resp=>{
                    if(resp==='succeed')
                    {
                        ...
                        const tableData={...datas,totalCount:this.state.tableData.totalCount}
                        this.setState({
                            tableData
                        })
                        console.log('删除成功')
    ...
    handleOk = () => {
        ...
        }else if(this.state.newPhone===this.state.user.phoneNumber&&this.state.newPassword===this.state.user.password)
        {
            this.setState({
        ...
        return (  
            ...
            <React.Fragment>
                ...
                <div className="input-group mb-3">
                    ...
                    <button className="btn btn-outline-secondary" type="button" onClick={this.searchUser}>查询</button>
                </div>
                ...
                <Modal title={this.state.user.userId} id='myModal'
                visible={this.state.is_show} onOk={this.handleOk} onCancel={()=>{this.setState({is_show:false})}}>
                    <form>
                    <div className="mb-3">
                        <label htmlFor="userId" className="form-label">原手机号 : {this.state.user.phoneNumber}</label>
                        <input onChange={(e)=>{this.setPhone(e.target.value)}} type="text" data-pure-clear-button
                         className="form-control" id="userId" placeholder='新手机号'/>
                    </div>
                    <div className="mb-3">
                        <label htmlFor="password" className="form-label">密码 : {this.state.user.password}</label>
                        <input onChange={(e)=>{this.setPassword(e.target.value)}} data-pure-clear-button
                        type="text" className="form-control" id="password" placeholder='新密码'/>
                    </div>
                    <div style={{height:'2rem',color:'red'}}>
                        {this.state.error_msg2}
                    </div>
                    <button onClick={()=>this.deleteOne(this.state.user)} type='submit' className="btn btn-primary" style={{width:'100%'}}>删除用户</button>
                    </form>
                </Modal>
                </BaseCord>
            </React.Fragment>
